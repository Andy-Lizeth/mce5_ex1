...
##2.1- Para su potencial investigación ¿Cuál sería su hipótesis ? (Explique)

***Respuesta:
Las especies que tienen baja afinidad alos suelos con el ambiente y la reducción de especies de una comunidad con los de la vegetación facilitarían amortiguar la pérdida de especies raras.
...
```{r crd_raraespecies}
sitios = c( "Archidona", "Tena" , "Cotundo" ,"Pacto Sumaco")
Archidona=sample(1:20, size=8)
Tena=sample(1:40, size=8)
Cotundo=sample(1:35,size=8)
PactoSumaco=sample(1:39, size=8)
datos=data.frame(sitios, Archidona, Tena, Cotundo, PactoSumaco)

write.csv(file="raraespecies_muestra.csv",datos)
```
Explique el resultado muestra: 

**Respuesta:**

***Se escogió estos 4 lugares debido a las diferentes condiciones  que se presentan lo cual podremos abordar diferentes resultados por ejemplo la cobertura de todas las especies de las plantas o observar la vegetación con otros individuso de la misma especie de una comunidad con las plantas que crecen solas en la vegetación, ver la abundancia del suelo .
 ...
 ***Mostrar que la facilitación favorece al establecimientos de especies raras que tien baja afinidad por los sustratos.También porque una vez que las plantas de maduran muestran la facilitación de reducción de rendimiento entre las especies de las comunidades

**Respuesta: ID

*** Aspectos críticos de la facilitación que pueden amortiguar la pérdida de especies.

## 2.4 Gráfica e hipótesis
...
<center>![](fotito.jpeg)</center>

Respuesta:

****Aquí nos representa lo puntos de todos los lugares seleccionados osea una correlación de una fuerza de interacción planta con planta (RII) y el índice de la afinidad del yeso que viene representado con (g).Lo cual se mostró una conexión negativa entre la facilitacion y la afinidad de los suelos de todas las especies raras en todas las localidades.
